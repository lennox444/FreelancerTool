...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Ungültige Anmeldedaten');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden isolate">
      ...
      <SpotlightCard
        className="w-full max-w-md mx-4 p-8 md:p-10 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-100"
        ...
        <form className="space-y-6 relative z-10" onSubmit={handleSubmit}>
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5 ml-1">
                ...
                <Link href="#" className="text-sm font-medium text-[#800040] hover:text-[#600030] transition-colors">
                  Vergessen?
                </Link>
              </div>
              <div className="relative group">
            ...
            <StarBorder as="button" className="w-full rounded-xl group" color="#d946ef" speed="4s" type="submit" disabled={loading}>
              <div className="w-full h-12 bg-[#800040] hover:bg-[#600030] text-white flex items-center justify-center rounded-xl transition-all font-semibold text-base shadow-lg shadow-pink-900/20">
                {loading ? (
                  ...
                  <>
                    Anmelden <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                  </>
                )}
              </div>
            </StarBorder>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white/95 text-slate-500 font-medium">Oder fortfahren mit</span>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-8">
            Noch keinen Account?{' '}
            <Link href="/register" className="font-semibold text-[#800040] hover:text-[#600030] transition-colors relative after:absolute after:bottom-0 after:left-0 after:h-[1px] after:w-0 hover:after:w-full after:bg-[#800040] after:transition-all after:duration-300">
